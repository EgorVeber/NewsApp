...
class FavoritesViewModel @Inject constructor(
            ...
            if (page == FavoritesViewPagerAdapter.FAVORITES) {
                        ...
                        val changedUiArticleList =
                            listArticle.reversed().map { article ->
                                article.publishedAtChange =
                                    stringFromData(article.publishedAt).toStringFormatDateUi()
                                article.viewType = BaseViewHolder.VIEW_TYPE_FAVORITES_NEWS
                                article
                            }.map { article -> article.toArticleUiModel() }
                ...
                viewModelScope.launchJob(tryBlock = {
                    ...
                    if (listArticle.isEmpty()) {
                        ...
                        listSave = favoritesInteractor.getListArticleGroupByDate(listArticle).toMutableList()
        ...
        articleUiModel.title?.let { title ->
            viewModelScope.launchJob(tryBlock = {
                ...
                val artModified = art.reversed()
                    .map { art ->
                        art.publishedAtChange = stringFromData(art.publishedAt).toStringFormatDateUi()
    ...
    fun clickGroupHistory(articleUiModel: ArticleUiModel) {
        ...
        Log.d("clickGroupHistory", "clickGroupHistory() called with: articleUiModel = $articleUiModel")
